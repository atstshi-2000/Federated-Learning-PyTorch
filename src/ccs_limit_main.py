...
if __name__ == '__main__':
    ...
    # クライアントごとのCPU使用率制限（例: cid=0は30%, cid=1は50%, ...）
    client_cpu_limits = {
        0: 30,
        1: 50,
        2: 70,
    ...
    for epoch in tqdm(range(args.epochs)):
       ...
       # ========== [2] (do_prune時) EL2Nスコア収集とCCS実行 ==========
        ...
        for cid in idxs_users:
            cpu_limit = client_cpu_limits.get(cid, 50)  # デフォルトで50%
            ...
            print(f"[Epoch {epoch+1}][Client {cid}] CPU制限: {cpu_limit}% にて学習開始")
            ...
            # モデル重みと損失の読み込み
            weights_path = f"weights/client_{cid}_epoch_{epoch}.pt"
            loss_path = f"weights/loss_{cid}_epoch_{epoch}.txt"

            # 読み込み処理
            if os.path.exists(weights_path) and os.path.exists(loss_path):
                ...
                print(f"[警告] Client {cid} のモデル or 損失ファイルが見つかりませんでした")
            # 出力を表示
            print(result.stdout)
            if result.stderr:
                print(f"Error (Client {cid}):", result.stderr)

            print(f"[Epoch {epoch+1}][Client {cid}] Train time: {t_train_end - t_train_start:.3f}s")
