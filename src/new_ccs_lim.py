...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for cid in idxs_users:
            ...
            #非ブロッキングで最新の CPU% を取得
            cpu_usages[cid] = psutil.Process().cpu_percent(interval=None)
            # 現在のメモリ使用量（RSS）を取得
            mem_usages[cid] = psutil.Process().memory_info().rss / (1024 ** 2)
            print(f"Client {cid} kept samples: {len(last_client_keep.get(cid, []))}")
